...
func (m *Matcher) Inspect(ctx context.Context, rc *pipeline.RequestContext) pipeline.Verdict {
	...
	for _, cm := range m.rules {
		...
		case config.ActionDisable:
			...
			rc.Values["matcher_rule"] = cm.rule.Name
			// Disable-hit audit follows the rule's log_enabled toggle like
			// every other matcher action (the console defaults it to on for
			// disable rules); publish the action and the full stage list for
			// the audit reason.
