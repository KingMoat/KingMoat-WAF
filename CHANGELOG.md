...
- **一键部署 `curl | bash` 主用法修复**：管道安装的重执行守卫此前自败——首次执行把管道内容写入临时副本后，第二遍仍从已耗尽的管道读入而重回守卫分支，且守卫内报错函数尚未定义，头部与 README 宣称的 `curl -fsSL … | bash` 一键安装必然失败；现以环境变量标记第二遍执行（不再重回守卫），守卫内报错改内联输出，临时脚本副本在退出时清理（含提前失败路径），不再残留 /tmp
